<template>
    <div style="background-color:  #1d1d1d; padding: 0;">
        <TopBar />
        <img src="https://images.fallenlondon.com/headers/prison.png" style="width: 100%;" />
            ...
            <div v-else style="width: 100%;">
                <Book style="justify-self: center; width: 80%;"  :Name="selectStory?.Name"
                 :GameId="selectStory?.GameId" :Description="selectStory?.Description"
                />
            </div>
  ...
  function playGame(msg: story) {
    shouldShowGlobalComponentd.value = false
    start.value = true
    selectStory.value=msg;
    console.log(selectStory.value.Name)
  }
  
  watch(()=>shouldShowGlobalComponentd.value,(newVal)=>{
  ...
  onMounted(async () => {
    await axios.get('/api/v1/game/story').then(response => {
